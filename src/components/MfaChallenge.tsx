...
import { verifyFactor, authenticatePasskey } from '../lib/mfa'
import { errorMessage } from '../lib/errors'
import { useAuth } from '../hooks/useAuth'
import { supabase } from '../lib/supabaseClient'

export function MfaChallenge() {
  const { mfaFactorId, mfaFactorType, clearMfaChallenge } = useAuth()
  ...
  async function handlePasskey() {
    ...
    setError(null)
    try {
      await authenticatePasskey(mfaFactorId)
      clearMfaChallenge()
    } catch (err) {
  ...
  const isPasskey = mfaFactorType === 'webauthn'
      ...
      <div className="w-full max-w-sm rounded-xl border border-line bg-panel p-8">
        <div className="mb-6 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-ink-muted">
            {isPasskey ? 'Verify with your passkey to continue' : 'Enter the 6-digit code from your authenticator app'}
          </p>
        </div>
        ...
        {isPasskey ? (
          <>
            ...
            >
              {submitting ? 'FOLLOW YOUR DEVICE PROMPT...' : 'VERIFY WITH PASSKEY'}
            </button>
          </>
        ) : (
